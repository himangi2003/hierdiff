...
"""TODO
Need to rewrite this so that each node is plotted as a pie chart,
but otherwise similar to hierdiff"""


def plot_nn_scatter(Z, title='', res=None, alpha_col='pvalue', alpha=0.05, tooltip_cols=[], colors=None, prune_col=None, leaf_counts=None):
    ...
    #jinja_env = Environment(loader=FileSy stemLoader(opj(_git, 'hierdiff', 'hierdiff')))
...
"""TODO: refactor and revise this code so that each point is a circle or pie chart
with associated school tip"""
def _nn_paths(Z, height, width, margin=10, res=None, alpha_col='pvalue', alpha=0.05, tooltip_cols=[], colors=None, min_count=0, prune_col=None, leaf_counts=None):
    if colors is None:
        colors = set1_colors
    lines = []
    annotations = []
